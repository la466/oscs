...
dir.create(file.path("graphs/simulation_synonymous_codon/"), showWarnings = FALSE)
...
excess_plot <- function(frame) {
  file <- read.csv('outputs/simulation_synonymous_codon_analysis/combined_stops.csv', head=T)
...
excess_plots <- function() {
  ...
  ggsave(file='graphs/simulation_synonymous_codon/osc_excesses.pdf', plots, width=8, height=12, device=cairo_pdf)
}
...
excess_codon_plot <- function(frame, codon, min=NULL, max=NULL) {
  file <- read.csv('outputs/simulation_synonymous_codon_analysis/all_codons.csv', head=T)
...
excess_individual_osc_plots <- function(min, max) {
  ...
  ggsave(file='graphs/simulation_synonymous_codon/osc_individual_excesses_+1_+2.pdf', plots, width=12, height=8)
}
...
osc_vioplots <- function() {
  ...
  ggsave(file='graphs/simulation_synonymous_codon/osc_individual_gc_vioplots.pdf', plots, width=12, height=8, device=cairo_pdf)
}
...
sink('outputs/r_outputs/synonymous_codon_model.txt')
