...
# Prerequisite file(s):
# Description:					Analysis for the codon shuffle model
...
simulation_output_directory = 'outputs/simulation_codon_shuffle/'
...
def check_values(real_counts):
    ...
    if real_counts[1]['AAA'] != 6.3488829763393895:
        print('+1 AAA fail')
        check = False
    if real_counts[2]['AAA'] != 4.830113800732595:
        print('+2 AAA fail')
        check = False
    if real_counts[1]['TTT'] != 4.086925342970879:
        print('+1 TTT fail')
        check = False
    if real_counts[2]['TTT'] != 6.943555977492472:
        print('+2 TTT fail')
        check = False
...
def write_stop_codons(results):
    ...
    output_file = open('outputs/simulation_codon_shuffle_analysis/stops.csv', 'w')
    header = 'acc,gc,gc3'
    for frame in frames:
...
def write_combined_stops(results):
    ...
    output_file = open('outputs/simulation_codon_shuffle_analysis/combined_stops.csv', 'w')
    header = 'acc,gc,gc3'
    for frame in frames:
...
def write_all_codons(results):
    ...
    output_file = open('outputs/simulation_codon_shuffle_analysis/all_codons.csv', 'w')
    header = 'acc,gc,gc3'
    for frame in frames:
...
def main():
    ...
    create_directory('outputs/simulation_codon_shuffle_analysis/')
