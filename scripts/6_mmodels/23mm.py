...
# Prerequisite file(s):
# Description:					Model sequences using second order Markov model
# Output files:
...
def get_usage(genome_seqs):
    ...
    for base1 in bases:
        for base2 in bases:
            usage[base1+'0'+base2+'1'] = {}
            usage[base1+'1'+base2+'2'] = {}
            usage[base1+'2'+base2+'0'] = {}
            norm_usage[base1+'0'+base2+'1'] = {}
            norm_usage[base1+'1'+base2+'2'] = {}
            norm_usage[base1+'2'+base2+'0'] = {}

            for base3 in bases:
                usage[base1+'0'+base2+'1'][base3] = 0
                usage[base1+'1'+base2+'2'][base3] = 0
                usage[base1+'2'+base2+'0'][base3] = 0
                norm_usage[base1+'0'+base2+'1'][base3] = 0
                norm_usage[base1+'1'+base2+'2'][base3] = 0
                norm_usage[base1+'2'+base2+'0'][base3] = 0
    ...
    for cds in genome_seqs:
        ...
        if count:
            ...
            # Use the first 2 nts of the query as starter
            for i in range(2,len(query_seq)-2):
                pos1 = query_seq[i]
                pos2 = query_seq[i+1]
                pos3 = query_seq[i+2]
                ...
                usage[pos1+index1+pos2+index2][pos3] += 1


    for pair in sorted(usage):
        for base in sorted(usage[pair]):
            norm_usage[pair][base] = (usage[pair][base]/sum(usage[pair].values()))
...
def get_off_frame_densities(seqs):
    ...
    for cds in seqs:
        ...
        codon_count += len(cds[3:-3])/3

        f1 = re.findall(r'.{3}', cds[4:])
        f2 = re.findall(r'.{3}', cds[5:])
        for codon in codon_list:
            off_frame_frequencies[1][codon] += f1.count(codon)
            off_frame_frequencies[2][codon] += f2.count(codon)
            off_frame_frequencies['both'][codon] += (f1.count(codon) + f2.count(codon))
    ...
    for repeat in repeat_list:
        ...
        for cds in genome_seqs:
            ...
            if gen_count:
                ...
                new_seq = cds[0:5]
                required_nts = len(cds) - 5 - 3  # minus start seq and stop codon
...
def write_to_file(accession, real_sample, results):
    ...
    output_file = open('outputs/mmodels/23mm/' + accession + '_23mm.csv', 'w')
...
def main():
    ...
    create_directory('outputs/mmodels/23mm/')
