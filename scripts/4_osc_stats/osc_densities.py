...
# Prerequisite file(s):
# Description:					Get OSC stats for genome sample
# Output files:					off_frame_densities.csv, combined_off_frame_densities.csv
...
def write_codon_densities(results):
    ...
    output_file = open('outputs/osc_densities/off_frame_densities.csv', 'w')
...
def write_combined_densities(results):
    ...
    output_file = open('outputs/osc_densities/combined_osc_densities.csv', 'w')
    header = 'acc,gc,gc3,trans_table,cds_count,codon_count'
    for frame in frames:
